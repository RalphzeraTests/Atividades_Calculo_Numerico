...
# Ideia: construir φ(x) que satisfaça as condi¸c˜oes do Teorema do Ponto Fixo.
# Linearizar: substituir f(x) pela reta tangente.
# Vamos impor que φ'(x∗) = 0. Como φ'(x) deve ser cont´ınua, existe I tal que k = max_x∈I |φ'(x)| < 1.
...
# φ(x) = x − f(x)/f'(x) ⇒ xn+1 = xn − f(xn)/f'(xn) , para i = 0, 1, ...
